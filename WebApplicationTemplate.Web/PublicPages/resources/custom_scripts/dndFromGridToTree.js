...
function ob_dnd_t11(event) {
    var mouseCoords = ob_t2_GetMouseCoords(event);
    if (ob_dnd_tva == true) {
        //		if(window.event)
        //		{
        ...
        //		}
        ob_dnd_tvo = mouseCoords[0];
        ob_dnd_tvp = mouseCoords[1];
        ob_dnd_tva = false;
        return;
    }
    else {
        ...
        //        }
        ob_dnd_tvs = mouseCoords[0];
        ob_dnd_tvd = mouseCoords[1];
    }
    ...
    //            notesDivCopy.style.left = event.x + ob_dnd_tvt - 12 + 'px';
    //            notesDivCopy.style.top = event.y + ob_dnd_tvq + 12 + 'px';
    //        }
    //    }
    //    else {
    ...
    //        if (notesDivCopy && ob_dnd_allowCopy) {
    //            // move the copy div at the mouse coords
    //            notesDivCopy.style.left = event.pageX - 12 + 'px';
    //            notesDivCopy.style.top = event.pageY + 12 + 'px';
    //        }
    //    }
...
//        }
    //    }
    var mouseCoords = ob_t2_GetMouseCoords(event);
    ob_dnd_tvh = mouseCoords[0];
    ob_dnd_tvj = mouseCoords[1];
...
function NotesKeyModifierWatch(event) {
    ...
    if (notesDivCopy == null) {
        ...
        notesDivCopy.style.left = '0px';
        notesDivCopy.style.top = '0px';
        notesDivCopy.id = ob_tree_id + '_copy_div';
        notesDivCopy.innerHTML = '<img src="../TreeView/icons/plus.gif">';
    ...
    if (ob_dnd_tvu != null) {
        notesDivCopy.style.left = ob_dnd_tvu.style.left.replace('px', '') * 1 - 5 + 'px';
        notesDivCopy.style.top = ob_dnd_tvu.style.top.replace('px', '') * 1 + 17 + 'px';
    }
...
function ob_getRecordIconSrc(oRecord) {
    return oRecord.firstChild.firstChild.firstChild.firstChild.src;
}
